...
const Contacts = () => {
    ...
    let [FromValue,setFromValue] = useState({fname : "",lname : "",email : "", msg : ""});
    ...
    const FormSubmit = async (e)=>{
        e.preventDefault();
        if(FromValue.fname.length === 0){
            toast.error("Frist Name Required!");
        }else if(FromValue.lname.length === 0){
            toast.error("Last Name Required!");
        }else if(FromValue.email.length === 0){
            toast.error("Email Required!");
        }else if(FromValue.msg.length === 0){
            toast.error("Message Required!");
        }else{
            const RawResponse = await axios.post("/api/contact",FromValue);
        }
    }
                                ...
                                <form onSubmit={FormSubmit} action="">
                                    ...
                                    <div className="mb-4 flex flex-wrap -mx-2">
                                        <div className="mb-4 lg:mb-0 w-full lg:w-1/2 px-2">
                                            <input value={FromValue.fname} onChange={(e)=>{inputOnChange("fname",e.target.value)}}  className="py-2 px-3 w-full bg-gray-50 rounded leading-loose" type="text" placeholder="First Name"/>
                                        </div>
                                        <div className="w-full lg:w-1/2 px-2">
                                            <input value={FromValue.lname} onChange={(e)=>{inputOnChange("lname",e.target.value)}} className="py-2 px-3 w-full bg-gray-50 rounded leading-loose" type="text" placeholder="Last Name"/>
                                        </div>
                                    </div>
                                    <input value={FromValue.email} onChange={(e)=>{inputOnChange("email",e.target.value)}} className="mb-4 py-2 px-3 w-full bg-gray-50 rounded leading-loose" type="email" placeholder="hello@example.com"/>
                                    <textarea value={FromValue.msg} onChange={(e)=>{inputOnChange("msg",e.target.value)}} className="mb-4 py-2 px-3 w-full bg-gray-50 rounded leading-loose" type="email" placeholder="Message"/>
